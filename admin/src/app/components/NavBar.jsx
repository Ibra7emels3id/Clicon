...
const NavBar = () => {
    ...
    // Icons Product
    const productIcon = (
        <svg className="w-6 h-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6">
    ...
    return (
        <div className="xl:w-64 w-16 border-r z-50 min-h-screen fixed text-base border-gray-300 pt-4 flex flex-col gap-1 transition-all duration-300">
            {sidebarLinks.map((item, index) => (
                <Link href={item.path} key={index}
                    className={`flex items-center py-3 px-4 gap-3  hover:bg-gray-100/90
                        ${(item.path === pathname || item.path === '/' + pathname.split("/")[1])
                            ? "border-r-4 md:border-r-[6px] bg-indigo-500/10 border-[#FA8232] text-[#FA8232]"
