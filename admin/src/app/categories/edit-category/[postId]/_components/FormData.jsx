...
import { EditCategory } from '@/lib/features/Category/EditCategorySlice';
import { FetchCategory } from '@/lib/features/Category/GetCategoryId';
...
const FormData = ({ id }) => {
    ...
    const HandleSubmitData = async (e) => {
        ...
        try {
            setLoading(true);
            const res = await dispatch(EditCategory({ id, formData })).unwrap();
            ...
            <div className="flex flex-col">
                ...
                <div className="flex flex-col py-5">
                    <form method="post" className='space-y-4'>
                        ...
                        <div className="image">
                            {formData?.image?.name ?
                                <Image src={URL.createObjectURL(formData?.image)} alt="uploadArea" width={100} height={100} /> :
                                <Image src={formData?.image} alt="image" width={100} height={100} />
                            }
                        </div>
                        <div className="title">
                            <input onChange={(e) => {
                                setFormData({ ...formData, category: e.target.value })
                            }} type="text" value={formData?.category} placeholder="Category Name" className='h-10 focus:outline-none w-full border border-gray-200 p-2' />
                        </div>
